...
def overwrite_python_analysis_extra_paths(isaaclab_settings: str) -> str:
    ...
    training_source_dir = os.path.join(PROJECT_DIR, "training", "source")
    isaaclab_extensions = os.listdir(training_source_dir)
    path_names.extend(['"${workspaceFolder}/training/source/' + ext + '"' for ext in isaaclab_extensions])
