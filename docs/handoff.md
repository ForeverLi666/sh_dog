...
- `SH_DOG_CFG` 使用 Go2 风格训练 PD `25/0.5`；`SH_DOG_STAND_CFG` 使用已验证站起 PD：
  abad `40/1.5`、hip `60/2`、knee `80/3`。
...
参考 Isaac Lab Go2 rough locomotion，建立 ShDog 平地速度跟踪任务和最小 PPO 配置；先完成零命令站立、
reset、接触、观测和 action scale 验证，再开始训练。
