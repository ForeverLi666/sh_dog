...
RS06 厂家过载表同样保存在 `model.toml`，数值均为电机模组输出侧事实。旋转工况使用
`17/200 s`、`20/36 s`、`25/18 s`、`30/8 s`、`36/4 s`；堵转工况连续边界为 `8 N·m`，
并使用 `11/200 s`、`17/15 s`、`25/5 s`、`30/1 s`、`36/1 s`。模型对厂家允许时间统一应用
`0.9` 安全系数，原始力矩与时间锚点不改写。
